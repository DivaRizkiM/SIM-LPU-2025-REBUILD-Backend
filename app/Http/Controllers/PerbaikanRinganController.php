...
class PerbaikanRinganController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $query = PencatatanKantor::select([
                'pencatatan_kantor.*',
                DB::raw("CASE WHEN pencatatan_kantor_kuis.id_tanya = 70 THEN kj.nama ELSE NULL END AS verifikasi"),
            ])
                ...
                ->where('pencatatan_kantor.jenis', 'Verifikasi Perbaikan Ringan')
            ...
            foreach ($result as $item) {
                ...
                $kesimpulan = ($item->verifikasi == 'Sesuai RAB' ? 'Melanjutkan Mendapatkan Subsidi Operasional LPU' : 'Tidak Diusulkan Mendapatkan Subsidi Operasional LPU');
                $data[] = [
                    ...
                    'hasil_verifikasi' => $item->verifikasi,
    ...
    public function export(Request $request)
    {
        try {
            ...
            $userLog=[
                'timestamp' => now(),
                'aktifitas' =>'Cetak Perbaikan Ringan',
                'modul' => 'Perbaikan Ringan',
                'id_user' => Auth::user(),
            ];
