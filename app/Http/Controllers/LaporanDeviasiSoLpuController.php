...
class LaporanDeviasiSoLpuController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $queryVerifikasiBiaya = DB::table('verifikasi_biaya_rutin_detail')
                ...
                ->groupBy('id_kpc');
            $queryAtribusi = DB::table('verifikasi_biaya_atribusi_detail')
                ->select('verifikasi_biaya_atribusi.id_kpc AS id_kpc', 'verifikasi_biaya_atribusi.tahun AS tahun', 'verifikasi_biaya_atribusi.triwulan AS triwulan')
                ->selectRaw('SUM(IF(id_verifikasi_biaya_atribusi,verifikasi,0)) AS verifikasi_atribusi')
    ...
    public function export(Request $request)
    {
        try {
            ...
            return Excel::download(new LaporanDeviasiSoLpuExport($data), 'template_laporan_deviasi_dana_lpu.xlsx');
        } catch (\Exception $e) {
