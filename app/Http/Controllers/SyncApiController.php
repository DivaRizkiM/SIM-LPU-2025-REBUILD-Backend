    ...
    public function syncKategoriBiaya(Request $request)
    {
        try {
           ...
           $payload = ApiRequestPayloadLog::create([
            ...
            return response()->json([
                'status' => 'SUCCESS',
                'message' => 'Sinkronisasi kategori biayar berhasil'], 200);
        } catch (\Exception $e) {
    ...
    public function syncDashboardProduksiPendapatan(Request $request)
{
    ...
    public function syncLampiran(Request $request)
    {
        try {
            ...
            // Query untuk mendapatkan data BiayaAtribusiDetail sesuai dengan parameter yang diberikanxxs
        ...
        // Tambahkan kondisi berdasarkan bulan jika ada
        if (!empty($bulan)) {
...
// dd($list);
            ...
            $userLog = [
                ...
                'modul' => 'Lampiran',
                'id_user' => auth()->id(), // Use auth() helper if this is a Laravel controller
            ];

            UserLog::create($userLog);
            $job = RsyncJob::dispatch($list, $endpoint,$totalData, $userAgent);
