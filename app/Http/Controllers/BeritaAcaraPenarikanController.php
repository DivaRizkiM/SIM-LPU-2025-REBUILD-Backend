...
class BeritaAcaraPenarikanController extends Controller
{
    ...
    public function pdf(Request $request)
    {
        try {
            ...
            // Query utama berdasarkan type_data
            $biaya = VerifikasiBiayaRutin::select([
                'verifikasi_biaya_rutin.*',
                'regional.nama as nama_regional',
                DB::raw($type_data == 1
                    ? 'SUM(verifikasi_biaya_rutin_detail.pelaporan) AS total_biaya_regional'
                    : 'SUM(verifikasi_biaya_rutin_detail.pelaporan_prognosa) AS total_biaya_regional'),
            ])
                ->leftJoin('verifikasi_biaya_rutin_detail', 'verifikasi_biaya_rutin_detail.id_verifikasi_biaya_rutin', '=', 'verifikasi_biaya_rutin.id')
                ->leftJoin('regional', 'regional.id', '=', 'verifikasi_biaya_rutin.id_regional')
                ->where('verifikasi_biaya_rutin.tahun', $tahun)
            ...
            $regionalIds = $biaya->pluck('id_regional');

            // Preload pendapatan untuk semua regional
            $pendapatanByRegional = ProduksiDetail::join('produksi', 'produksi.id', '=', 'produksi_detail.id_produksi')
                ->select('produksi.id_regional', DB::raw('SUM(' . ($type_data == 1 ? 'produksi_detail.pelaporan' : 'ROUND(produksi_detail.pelaporan_prognosa)') . ') as total_pendapatan'))
            ...
            // Pasangkan pendapatan ke setiap biaya
            foreach ($biaya as $item) {
                $item->pendapatan_regional = $pendapatanByRegional[$item->id_regional] ?? 0;
                $item->total_biaya_regional_terbilang = $this->convertToRupiahTerbilang($item->total_biaya_regional);
            }

            // Query pendapatan untuk keseluruhan produksi
            $pendapatan = Produksi::select([
                'produksi.*',
                DB::raw('SUM(' . ($type_data == 1 ? 'produksi_detail.pelaporan' : 'produksi_detail.pelaporan_prognosa') . ') AS total_regional'),
            ])
                ->leftJoin('produksi_detail', 'produksi_detail.id_produksi', '=', 'produksi.id')
                ->groupBy('produksi.triwulan', 'produksi.id_regional')
                ->get();
            ...
            foreach ($pendapatan as $item) {
                $item->total_pendapatan_regional_terbilang = $this->convertToRupiahTerbilang($item->total_regional);
            }

            // Proses tanggal kuasa
            $tanggalCarbon = Carbon::createFromFormat('Y-m-d', $tanggal);
            ...
            // User Identity
            $user = Auth::user();
            $user_identity = $user->id_kpc ? Kpc::find($user->id_kpc) : ($user->id_kprk ? Kprk::find($user->id_kprk) : Regional::find($user->id_regional));
