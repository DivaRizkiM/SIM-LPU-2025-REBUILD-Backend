...
use App\Exports\LaporanPrognosaBiayaExport;
use App\Models\Kpc;
...
class LaporanPrognosaBiayaController extends Controller
{
        ...
        try {
            ...
            foreach ($result as $value) {
                $verifikasiQuery = DB::table('verifikasi_biaya_rutin')
                    ...
                    ->selectRaw('(SUM(verifikasi_biaya_rutin_detail.pelaporan_prognosa)) AS hasil_verifikasi')
    ...
    public function export(Request $request)
    {
        try {
            ...
            $userLog=[
                'timestamp' => now(),
                'aktifitas' =>'Cetak Laporan Prognosa Biaya',
                'modul' => 'Laporan Prognosa Biaya',
                'id_user' => Auth::user(),
            ];

            $userLog = UserLog::create($userLog);
            return Excel::download(new LaporanPrognosaBiayaExport($datas), 'laporan_verifikasi_biaya.xlsx');
        } catch (\Exception $e) {
            return response()->json(['status' => 'ERROR', 'message' => $e->getMessage()], 500);
        }
    }
