...
class LaporanRealisasiSoLpuController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $subquery_vba = DB::table('verifikasi_biaya_atribusi_detail')
                ->select(
                    'verifikasi_biaya_atribusi.id_kpc as id_kpc',
                    'verifikasi_biaya_atribusi.tahun AS tahun',
                    'verifikasi_biaya_atribusi.triwulan AS triwulan',
                    DB::raw('SUM(IF(id_verifikasi_biaya_atribusi,verifikasi,0)) AS verifikasi_atribusi')
                )
    ...
    public function export(Request $request)
    {
        try {
            ...
            //             ->on('verifikasi_biaya.triwulan', '=', 'produksi.triwulan');
            //     })
            //     ->leftJoin(DB::raw('(SELECT verifikasi_biaya_atribusi.id_kpc as id_kpc, verifikasi_biaya_atribusi.tahun AS tahun, verifikasi_biaya_atribusi.triwulan AS triwulan,
            //         SUM(IF(id_verifikasi_biaya_atribusi,verifikasi,0)) AS verifikasi_atribusi
            //         FROM verifikasi_biaya_atribusi_detail
            //         LEFT JOIN verifikasi_biaya_atribusi ON verifikasi_biaya_atribusi.id = verifikasi_biaya_atribusi_detail.id_verifikasi_biaya_atribusi
            //         GROUP BY id_kpc
            ...
            return Excel::download(new LaporanRealisasiSoLpuExport($data), 'template_laporan_realisasi_dana_lpu.xlsx');
        } catch (\Exception $e) {
            return response()->json(['status' => 'ERROR', 'message' => $e->getMessage()], 500);
        }
    }
