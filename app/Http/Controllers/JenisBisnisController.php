...
class JenisBisnisController extends Controller
{
    public function index()
    {
        try {
            ...
            $total_data = $jenis_bisnissQuery->count();
            if ($search !== '') {
                $jenis_bisnissQuery->where('nama', 'like', "%$search%");
            }

            $jenis_bisniss = $jenis_bisnissQuery->offset($offset)
                ->limit($limit)->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json(['status' => 'SUCCESS', 'data' => $jenis_bisnis], 201);
        } catch (\Exception $e) {
            return response()->json(['status' => 'ERROR', 'message' => $e->getMessage()], 500);
        }
    }
